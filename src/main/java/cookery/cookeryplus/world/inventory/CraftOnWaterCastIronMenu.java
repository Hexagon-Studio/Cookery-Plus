...
package cookery.cookeryplus.world.inventory;
...
import cookery.cookeryplus.network.CraftOnWaterCastIronSlotMessage;
import cookery.cookeryplus.init.CookeryModMenus;
import cookery.cookeryplus.CookeryMod;

public class CraftOnWaterCastIronMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public CraftOnWaterCastIronMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(CookeryModMenus.CRAFT_ON_WATER_CAST_IRON.get(), id);
		...
		this.customSlots.put(5, this.addSlot(new SlotItemHandler(internal, 5, 34, 53) {
		}));
	...
	private void slotChanged(int slotid, int ctype, int meta) {
		if (this.world != null && this.world.isClientSide()) {
			CookeryMod.PACKET_HANDLER.sendToServer(new CraftOnWaterCastIronSlotMessage(slotid, x, y, z, ctype, meta));
			CraftOnWaterCastIronSlotMessage.handleSlotAction(entity, slotid, ctype, meta, x, y, z);
		}
	}
